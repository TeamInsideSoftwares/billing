    ...
    <form method="POST" action="{{ route('quotations.update', $quotation) }}" class="client-form">
        ...
        <div class="form-grid">
            <div>
                ...
                <select id="clientid" name="clientid" required>
                    ...
                    <optgroup label="{{ $group === 'regular' ? 'Regular Clients' : 'Prospect Clients' }}">
                        ...
                        <option value="{{ $client->clientid }}" {{ old('clientid', $quotation->clientid) ==
            ...
            <div>
                ...
                <input type="text" id="quo_number" name="quo_number"
                    value="{{ old('quo_number', $quotation->quo_number) }}" required>
            </div>
            <div>
                <label for="issue_date">Issue Date *</label>
                <input type="date" id="issue_date" name="issue_date" min="{{ $quotationDateBounds['min_date'] }}"
                    max="{{ $quotationDateBounds['issue_max_date'] ?? $quotationDateBounds['max_date'] }}"
                    value="{{ old('issue_date', optional($quotation->issue_date)->format('Y-m-d') ?: $quotationDateBounds['default_issue_date']) }}"
                    required>
            </div>
            <div>
                <label for="due_date">Due Date</label>
                <input type="date" id="due_date" name="due_date" min="{{ $quotationDateBounds['min_date'] }}"
            ...
            <div>
                ...
                <select id="status" name="status">
                    ...
                    <option value="{{ $status }}" {{ old('status', $quotation->status) == $status ? 'selected' : ''
