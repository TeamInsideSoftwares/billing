...
<script>
    (function () {
        ...
        window.refreshTemplateFromCampio = async function (channel) {
            ...
            const payload = options.length > 0 ? options[0] : null;
            const templateId = payload ? payload.templateid : null;

            if (!templateId) {
                alert('No template configured for this channel.');
                return;
            }
            ...
            try {
                ...
                const response = await fetch('{{ route("message-templates.refresh") }}', {
                    ...
                    body: JSON.stringify({
                        ...
                        document_id: '{{ $quotation->quotationid }}'
                    })
                });
                const data = await response.json();
                if (data.success) {
                    if (typeof templateCatalog !== 'undefined' && templateCatalog[channel]) {
                        const catalogEntry = templateCatalog[channel].find(t => t.templateid == templateId);
                        if (catalogEntry) {
