...
<div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem;">
                ...
                <div style="padding: 1rem; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                    ...
                    <form method="POST" action="{{ route('account.billing.update') }}" id="proforma-serial-form">
                        @csrf
    ...
    <!-- Quotation Serial Config -->
                <div style="padding: 1rem; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
                    ...
                    <form method="POST" action="{{ route('account.quotation.update') }}" id="quotation-serial-form">
                        ...
                        @php
                            $quotationPreview = $editingQuotationDetail?->generateNextSerialNumber() ?: 'Configure serial first';
                        @endphp
                        <div style="margin-bottom: 1rem;">
                            <label style="font-weight: 600; font-size: 0.7rem; color: #64748b; text-transform: uppercase;">Preview</label>
                            <div id="quotation-preview" style="font-family: monospace; font-size: 1rem; font-weight: bold; color: {{ empty($editingQuotationDetail) ? '#94a3b8' : '#1e293b' }}; padding: 0.5rem; background: white; border-radius: 6px; border: 2px dashed {{ empty($editingQuotationDetail) ? '#94a3b8' : '#cbd5e1' }}; text-align: center; margin-top: 0.25rem;">
                                {{ $quotationPreview }}
                            </div>
                        </div>
