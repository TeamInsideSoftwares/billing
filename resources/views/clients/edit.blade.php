    ...
    <form method="POST" action="{{ route('clients.update', $client) }}" class="client-form" enctype="multipart/form-data">
        ...
        <div class="form-grid" style="grid-template-columns: repeat(4, 1fr); gap: 0.75rem; margin-bottom: 1.5rem;">
            <div style="grid-column: span 2;">
                ...
                <input type="text" id="business_name" name="business_name" value="{{ old('business_name', $client->business_name) }}" required style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
            ...
            <div>
                ...
                <input type="text" id="contact_name" name="contact_name" value="{{ old('contact_name', $client->contact_name) }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email', $client->email) }}" required style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                @error('email') <span class="error">{{ $message }}</span> @enderror
            </div>
            <div>
                <label for="phone" style="font-size: 0.8rem;">Phone</label>
                <input type="tel" id="phone" name="phone" value="{{ old('phone', $client->phone) }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
            </div>
            <div>
                <label for="whatsapp_number" style="font-size: 0.8rem;">WhatsApp</label>
                <input type="tel" id="whatsapp_number" name="whatsapp_number" value="{{ old('whatsapp_number', $client->whatsapp_number) }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
            </div>
            <div>
            ...
            <div>
                ...
                @if($client->logo_path)
                    <div style="margin-top: 0.35rem;"><img src="{{ $client->logo_path }}" alt="Logo" style="height: 32px; border-radius: 4px;"></div>
                @endif
        ...
        <div class="form-grid" style="grid-template-columns: repeat(4, 1fr); gap: 0.75rem; margin-bottom: 1.5rem;">
            <div>
                ...
                <select id="country" name="country" class="country-select" data-selected="{{ old('country', $client->country ?? 'India') }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                    <option value="">Select</option>
                </select>
            </div>
            <div>
                <label for="state" style="font-size: 0.8rem;">State *</label>
                <select id="state" name="state" required class="state-select" data-selected="{{ old('state', $client->state) }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                    <option value="">Select</option>
                </select>
            ...
            <div>
                ...
                <select id="city" name="city" class="city-select" data-selected="{{ old('city', $client->city) }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                    <option value="">Select</option>
                </select>
            </div>
            <div>
                <label for="postal_code" style="font-size: 0.8rem;">Postal Code</label>
                <input type="text" id="postal_code" name="postal_code" value="{{ old('postal_code', $client->postal_code) }}" maxlength="20" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
            </div>
            <div style="grid-column: span 2;">
                <label for="address_line_1" style="font-size: 0.8rem;">Address</label>
                <textarea id="address_line_1" name="address_line_1" rows="2" maxlength="300" style="resize: vertical; font-size: 0.85rem; padding: 0.45rem 0.6rem;">{{ old('address_line_1', $client->address_line_1) }}</textarea>
            </div>
        </div>
        ...
        <div id="new-billing-fields">
            <div class="form-grid" style="grid-template-columns: repeat(4, 1fr); gap: 0.75rem;">
                <div style="grid-column: span 2;">
                    ...
                    <input type="text" id="billing_business_name" name="billing_business_name" value="{{ old('billing_business_name', $client->billingDetail->business_name ?? $client->business_name) }}" maxlength="150" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                ...
                <div>
                    ...
                    <input type="email" id="billing_email" name="billing_email" value="{{ old('billing_email', $client->billingDetail->billing_email ?? '') }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                ...
                <div>
                    ...
                    <select id="billing_country" name="billing_country" class="country-select" data-selected="{{ old('billing_country', $client->billingDetail->country ?? 'India') }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                        <option value="">Select</option>
                    </select>
                </div>
                <div>
                    <label for="billing_state" style="font-size: 0.8rem;">State *</label>
                    <select id="billing_state" name="billing_state" class="state-select" data-selected="{{ old('billing_state', $client->billingDetail->state ?? '') }}" required style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                        <option value="">Select</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="billing_city" name="billing_city" class="city-select" data-selected="{{ old('billing_city', $client->billingDetail->city ?? '') }}" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                        <option value="">Select</option>
                    </select>
                </div>
                <div>
                    <label for="billing_postal_code" style="font-size: 0.8rem;">Postal Code</label>
                    <input type="text" id="billing_postal_code" name="billing_postal_code" value="{{ old('billing_postal_code', $client->billingDetail->postal_code ?? '') }}" maxlength="20" style="font-size: 0.85rem; padding: 0.45rem 0.6rem;">
                </div>
                <div style="grid-column: span 2;">
                    <label for="billing_address_line_1" style="font-size: 0.8rem;">Billing Address</label>
                    <textarea id="billing_address_line_1" name="billing_address_line_1" rows="2" maxlength="300" style="resize: vertical; font-size: 0.85rem; padding: 0.45rem 0.6rem;">{{ old('billing_address_line_1', $client->billingDetail->address_line_1 ?? '') }}</textarea>
        ...
        <div class="form-actions" style="margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid #e5e7eb;">
            <button type="submit" class="primary-button" style="padding: 0.5rem 1.5rem; font-size: 0.9rem;">Update Client</button>
