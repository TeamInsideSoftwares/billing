    ...
    <form method="POST" action="{{ route('clients.store') }}" class="client-form" enctype="multipart/form-data">
        ...
        <div class="form-grid grid-cols-4">
            <div class="col-span-2">
                <label for="business_name" class="label-small">Business Name *</label>
            ...
            <div>
                <label for="contact_name" class="label-small">Contact Person</label>
                <input type="text" id="contact_name" name="contact_name" value="{{ old('contact_name') }}">
            </div>
            <div>
                <label for="groupid" class="label-small">Group</label>
                <div class="input-row">
                    <select id="groupid" name="groupid" class="flex-fill">
            ...
            <div>
                <label for="email" class="label-small">Email *</label>
                <input type="email" id="email" name="email" value="{{ old('email') }}" required>
                @error('email') <span class="error">{{ $message }}</span> @enderror
            </div>
            <div>
                <label for="phone" class="label-small">Phone</label>
                <input type="tel" id="phone" name="phone" value="{{ old('phone') }}">
            </div>
            <div>
                <label for="whatsapp_number" class="label-small">WhatsApp</label>
                <input type="tel" id="whatsapp_number" name="whatsapp_number" value="{{ old('whatsapp_number') }}">
            </div>
            <div>
                <label for="status" class="label-small">Status</label>
            ...
            <div>
                <label for="currency" class="label-small">Currency *</label>
            ...
            <div>
                <label for="logo" class="label-small">Logo</label>
        ...
        <div class="form-grid grid-cols-4">
            <div>
                <label for="country" class="label-small">Country</label>
                <select id="country" name="country" class="country-select" data-selected="{{ old('country', 'India') }}">
                    <option value="">Select</option>
                </select>
            </div>
            <div>
                <label for="state" class="label-small">State *</label>
                <select id="state" name="state" required class="state-select" data-selected="{{ old('state') }}">
                    <option value="">Select</option>
                </select>
                @error('state') <span class="error">{{ $message }}</span> @enderror
            </div>
            <div>
                <label for="city" class="label-small">City</label>
                <select id="city" name="city" class="city-select" data-selected="{{ old('city') }}">
                    <option value="">Select</option>
                </select>
            </div>
            <div>
                <label for="postal_code" class="label-small">Postal Code</label>
                <input type="text" id="postal_code" name="postal_code" value="{{ old('postal_code') }}" maxlength="20">
            </div>
            <div class="col-span-2">
                <label for="address_line_1" class="label-small">Address</label>
                <textarea id="address_line_1" name="address_line_1" rows="2" maxlength="300" class="textarea-auto">{{ old('address_line_1') }}</textarea>
            </div>
        </div>
        ...
        <div class="panel-note">
            <div class="flex-between">
                <label for="existing_bd_id" class="label-small">Use existing billing profile</label>
        ...
        <div id="new-billing-fields">
            <div class="form-grid grid-cols-4">
                <div class="col-span-2">
                    <label for="billing_business_name" class="label-small">Billing Business Name *</label>
                    <input type="text" id="billing_business_name" name="billing_business_name" value="{{ old('billing_business_name') }}" maxlength="150">
                    @error('billing_business_name') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="billing_gstin" class="label-small">GSTIN</label>
                    <input type="text" id="billing_gstin" name="billing_gstin" value="{{ old('billing_gstin') }}"
                       ...
                       >
                    <span id="gstin_hint" class="text-xs">Exactly 15 characters required</span>
                    @error('billing_gstin') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="billing_email" class="label-small">Billing Email</label>
                ...
                <div>
                    <label for="billing_phone" class="label-small">Billing Phone</label>
                ...
                <div>
                    <label for="billing_country" class="label-small">Country</label>
                    <select id="billing_country" name="billing_country" class="country-select" data-selected="{{ old('billing_country', 'India') }}">
                        <option value="">Select</option>
                    </select>
                </div>
                <div>
                    <label for="billing_state" class="label-small">State *</label>
                    <select id="billing_state" name="billing_state" class="state-select" data-selected="{{ old('billing_state') }}" required>
                        <option value="">Select</option>
                    </select>
                    @error('billing_state') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="billing_city" class="label-small">City</label>
                    <select id="billing_city" name="billing_city" class="city-select" data-selected="{{ old('billing_city') }}">
                        <option value="">Select</option>
                    </select>
                </div>
                <div>
                    <label for="billing_postal_code" class="label-small">Postal Code</label>
                    <input type="text" id="billing_postal_code" name="billing_postal_code" value="{{ old('billing_postal_code') }}" maxlength="20">
                </div>
                <div class="col-span-2">
                    <label for="billing_address_line_1" class="label-small">Billing Address</label>
                    <textarea id="billing_address_line_1" name="billing_address_line_1" rows="2" maxlength="300" class="textarea-auto">{{ old('billing_address_line_1') }}</textarea>
                ...
                <form id="groupForm" method="POST" action="{{ route('groups.store') }}" class="panel-note">
                    ...
                    <div class="grid-cols-2">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="group_name" id="groupName" required maxlength="150">
                        </div>
                        <div class="col-span-2">
                            <label class="label-compact">Email</label>
                            <input type="email" name="email" id="groupEmail" maxlength="150">
                        </div>
                        <div class="col-span-2">
                            <label class="label-compact">Address Line 1</label>
                            <input type="text" name="address_line_1" id="groupAddress1" maxlength="150">
                        </div>
                        <div class="col-span-2">
                            <label class="label-compact">>Address Line 2</label>
                            <input type="text" name="address_line_2" id="groupAddress2" maxlength="150">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="postal_code" id="groupPostalCode" maxlength="20">
                        </div>
                    </div>
                    <div class="flex-between">
